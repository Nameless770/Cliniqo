...
| `requestId`        | **Do not pass it.** Resolved by the writer itself — see below.                                                                               |

### Request correlation is not your job

`writeAuditEvent` stamps `request_id` on every row by reading the id middleware issued for
the request being served. Callers pass nothing.

That is deliberate. There are twenty-six call sites across eleven modules, and an id
threaded by hand is one a future call site forgets — producing a row that looks perfectly
well-formed and simply cannot be grouped with the others from its request. The same
reasoning that put authorization and the audit write inside this layer rather than at each
caller applies to the id.

What it buys: one chart open writes several rows across several actions, and "what did this
single request touch" becomes one indexed predicate. Outside a request — the maintenance
script, the integration tests — it resolves to null rather than throwing. A missing
correlation id must never cost an audit row; the row is the record, the id only makes it
easier to group.

The id is generated by middleware and **never read from the inbound request**. Honouring a
client-supplied `x-request-id` would let a caller stitch their reads onto someone else's
reference, in a table retained for six years and read as evidence.
...
Break-glass was listed here as modelled-but-unwired for several phases. It is wired now: a
clinician requests a grant with a stated reason, an administrator reviews it, and an active
grant lifts the PHI read budget. See `data-access/break-glass.ts`.
